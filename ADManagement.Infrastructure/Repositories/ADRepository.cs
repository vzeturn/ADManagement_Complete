...
public class ADRepository : IADRepository
{
    ...
    private ADUser MapUserPrincipalToADUser(UserPrincipal user)
    {
        ...
        return new ADUser
        {
            ...
            MemberOf = user.GetGroups().Select(g => g.Name).ToList(),
            
            // Additional properties from DirectoryEntry
            Department = entry?.Properties["department"].Value?.ToString() ?? string.Empty,
            Title = entry?.Properties["title"].Value?.ToString() ?? string.Empty,
            Company = entry?.Properties["company"].Value?.ToString() ?? string.Empty,
            Office = entry?.Properties["physicalDeliveryOfficeName"].Value?.ToString() ?? string.Empty,
            PhoneNumber = entry?.Properties["telephoneNumber"].Value?.ToString() ?? string.Empty,
            MobileNumber = entry?.Properties["mobile"].Value?.ToString() ?? string.Empty
        };
    }
